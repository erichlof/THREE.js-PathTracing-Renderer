...
* [Naive Path Tracing Hidden-Light comparison Demo](https://erichlof.github.io/THREE.js-PathTracing-Renderer/Compare_Uni-Directional.html) <br>
...
* [Bi-Directional Path Tracing Hidden-Light comparison Demo](https://erichlof.github.io/THREE.js-PathTracing-Renderer/Compare_Bi-Directional_PathTracing.html) <br>
...
* [Liquid Simulation Demo](https://erichlof.github.io/THREE.js-PathTracing-Renderer/Liquid_Simulation.html)<br>
...
* July 26th, 2017: Sorry for the recent slowness in code updates - I have been experimenting with path tracing noise/fBm(fractal Brownian motion), both procedural and texture-based.  I updated the ocean/sky model demo to include procedural clouds based on noise/fBm routines.  The cloud coverage can be stationary or dynamic, and the clouds themselves can be thin or thick.  I was originally aiming for cloud rendering like in the Horizon Zero Dawn game by Guerrilla Games, but their shader code is not public.  I was however able to study their volumetric cloud presentation at SIGGRAPH and at least get some ideas of how clouds form and appear in nature.  My clouds are not as realistic as theirs, but in this ocean setting, I'm happy with them at the moment.  If I find any improvements, I will update the ocean/sky demo.  I think my next project will now be to apply the noise/fBm routines to terrain rendering (hills, mountains, etc.).  Again, sorry for the lack in updates, this noise/fBm rendering is difficult for me to grasp, and it's just me working on it! ;-) 
...
* Add support for loading textures from files and applying them to materials and layered materials
* Instead of scene description hard-coded in the path tracing shader, let the scene be defined using the Three.js library
* Debug BVH triangle rendering - figure out a way around dynamic array index ban in WebGL 1.0, or wait until 2.0 in three.js
