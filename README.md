...
* September 26th, 2018: The BVH for WebGL 1.0 is finally working!  Well, for the most part anyway. The WebGL 1.0 version works great for lower than 800-triangle models - 60fps on desktop and 30fps on my smartphone, but as soon as I tried heftier models of just a little over that, say 1000 triangles, it either will not compile, or I can see the rendered model for a brief moment (in all its glory) before the frame rate drops to 0, the browser tab crashes, and the WebGL context is lost, so you have to close and reopen the tab which is annoying.  I have a couple of ideas of why this is happening - one being all the 'if' statements in the workarounds that the GPU has to deal with.  I'll have to investigte further.  So now I'm at a fork in the road.  I can try debugging the BVH for WebGL 1.0 with all its limitations, or go with WebGL 2.0, which support for was just realeased in the newest version of three.js r97, yay! It would supprt dynamic array indexing, getting rid of all those 'if' statements in the workarounds. This sounds simple, just call WebGL 2.0 renderer from three.js setup code - but it is more involved. Since WebGL 2.0 uses OpenGL ES 3.0 (I know confusing right?) all of my established path tracing shader code here on this repo will not compile right away. I have to manually go in and change stuff like 'attribute' to 'in' and 'varying' to 'in' and 'out', among other things. So once I can get WebGL 2.0 working, I can see if dynamic array indexing under WebGL 2.0 helps my crashing issue. As always I'll keep you updated with my progress. Sorry it has been quiet around this repo the last couple of months, I have been wrestling with BVHs and WebGL 1.0 limitations and the like!
