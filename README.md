...
* April 16th, 2019: Improved and expanded support/features for quadric (mathematical) shapes including ellipsoids(perfect spheres also), cylinders, cones, paraboloids (bowl/vase shape), hyperboloids (hourglass shape) and hyperbolic paraboloids (saddle shape or Pringles potato chips shape, ha).  Instead of trying to intersect these shapes in world space, like I used to, I instead define them all as generalized shapes with a radius of 1 (unit sphere, unit cylinder, unit cone, etc), and then transform the ray into their object space.  Once in object space, it is easier to intersect quadric shapes because they are located and centered at 0,0,0 (the origin), they are non-rotated, and they all have a unit size, or a radius of 1. When an intersection is found, the normal vector is transformed back into world space (via a special normal matrix) and the resulting image shows the scaled, rotated, translated object just like it had been there all along!  It's one of raytracing's best tricks.  Since everything is in object space, I was also able to add support for cutting sections out of the shape, and parameterizing some of the coeffecient constants (like the thickness of the hourglass hyperboloid shape).  I also use this object space trick on the gltf models with multiple thousands of triangles, which allows the entire model with all its triangles to rotate, translate, and arbitrarily scale in real time at 60fps!  
